...
*  @param type       只顯示指定型態的分類，可傳的值為 folder 或 category
*  @param completion succeed = YES 時 result 可以用，succeed = NO 時 result 會是 nil，錯誤原因會在 NSError 物件中
*/
...
- (void)deleteBlogCommentWithCommentID:(NSString *)commentID
                            completion:(PIXHandlerCompletion)completion __deprecated_msg("Please use -deleteBlogComments:completion: instead of this method.");
/**
